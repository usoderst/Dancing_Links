...
def GenQue(QueFilename,Nkeys,path):
    keyque = []
    Data_File = open(pathQues + QueFilename, "r")
...
class Multi_Graph:
    ...
    def DancingLinks(self,nodeArrayIn,valueArrayIn):
        ...
        if Qswitch == True: #Select a node based on the ques
            for qfile in os.listdir(pathQues):
                if qfile != ".DS_Store":
                    if qfile == ("DegreeQue" + filename):
                        Que = GenQue(qfile,Nkeys,pathQues)
                        levelNodeKey = Que.pop(0)
                        levelNode = self.vertList[levelNodeKey]

        if Qswitch == False:   #randomly select a node, but still open files to standardized processes
            for qfile in os.listdir(pathQues):
                if qfile != ".DS_Store":
                    if qfile == ("DegreeQue" + filename):
                        Que = GenQue(qfile,Nkeys,pathQues)
...
path = "/Users/Ulrik/Desktop/NetworksProject/TestBig/"
pathQues = "/Users/Ulrik/Desktop/NetworksProject/DegreeQues/"
    ...
    if filename != ".DS_Store":
        if county <= 13:
            ...
            for i in range(10):
                ...
                print("\n")
                RRuns.append(str(end_time - start_time))
            county += 1 
    ...
    if filename != ".DS_Store":
        if county <= 13:
            ...
            for i in range(10):
                ...
                print("------Runtime: " + str(end_time - start_time) + ' seconds------')
                print("\n")
                QRuns.append(str(end_time - start_time))
            county += 1 
